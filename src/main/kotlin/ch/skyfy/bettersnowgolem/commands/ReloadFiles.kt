package ch.skyfy.bettersnowgolem.commands
...
class ReloadFilesCmd : Command<ServerCommandSource> {
    ...
    companion object {
        fun register(dispatcher: CommandDispatcher<ServerCommandSource>) {
            val cmd = literal("bettersnowgolem").requires { source -> source.hasPermissionLevel(4) }
                .then(
                    literal("reload").then(
    ...
    override fun run(context: CommandContext<ServerCommandSource>): Int {
        ...
        if (list.contains(false)) {
            ...
            BetterSnowGolemMod.LOGGER.warn("Configuration could not be reloaded")
        } else {
            context.source.sendFeedback(Text.literal("The configuration was successfully reloaded"), false)
            BetterSnowGolemMod.LOGGER.info("The configuration was successfully reloaded")
        }
